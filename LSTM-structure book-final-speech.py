...
batch_size = 800 # Number of samples in a batch
num_nodes = [800,300,100,30] # Number of hidden nodes in each layer of the deep LSTM stack we're using
...
split_outputs = tf.nn.softmax(split_outputs)
...
with tf.control_dependencies([tf.assign(c[li], state[li][0]) for li in range(n_layers)]+
  ...
  for ui in range(num_unrollings):
    correct_prediction = tf.equal(tf.argmax(split_outputs[ui], 1), tf.argmax(train_outputs[ui], 1))
    xentropy = tf.nn.softmax_cross_entropy_with_logits(logits=split_outputs[ui], labels=train_outputs[ui])
    loss_ = tf.reduce_mean(xentropy)/batch_size
...
optimiz = tf.train.AdamOptimizer(learning_rate)
optimizer = optimiz.minimize(loss_, global_step=global_step)
#gradients, v = zip(*optimizer.compute_gradients(loss_))
...
for ui,(dat,lbl) in enumerate(zip(test_data,test_label)):
    feed_dict[train_inputs[ui]] = dat
    feed_dict[train_outputs[ui]] = lbl
